...
int main(void) {
    blb_blob_t *blob = blb_blob_create(4, 1);
    if (!blob) {
        return 1;
    }

    blb_blob_jump(blob, 0);
    blb_blob_put(blob, 0x24);

    blb_blob_jump(blob, 1);
    blb_blob_put(blob, 0xfe);

    blb_blob_jump(blob, 2);
    blb_blob_put(blob, 0xae);

    blb_blob_jump(blob, 3);
    blb_blob_put(blob, 0xdb);

    uint8_t value = 0;
    if (blb_blob_jump(blob, 0) && blb_blob_get(blob, &value)) {
        printf("value = %02x\n", value);
    }
